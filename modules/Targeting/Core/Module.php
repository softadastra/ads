...
return new class implements ModuleContract {
    ...
    {
        return 'Targeting/Core';
    }

    public function register(): void
    {
        $path = __DIR__ . '/Config/' . strtolower('Targeting') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Targeting'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Targeting')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Targeting'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Targeting')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Targeting'), __DIR__ . '/views');
