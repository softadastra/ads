...
return new class implements ModuleContract {
    ...
    {
        $path = __DIR__ . '/Config/' . strtolower('Campaign') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Campaign'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Campaign')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Campaign'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Campaign')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Campaign'), __DIR__ . '/views');
