...
return new class implements ModuleContract {
    ...
    {
        $path = __DIR__ . '/Config/' . strtolower('Creative') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Creative'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Creative')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Creative'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Creative')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Creative'), __DIR__ . '/views');
