...
return new class implements ModuleContract {
    ...
    {
        $path = __DIR__ . '/Config/' . strtolower('Delivery') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Delivery'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Delivery')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Delivery'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Delivery')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Delivery'), __DIR__ . '/views');
