...
return new class implements ModuleContract {
    ...
    {
        return 'Promotion/Core';
    }

    public function register(): void
    {
        $path = __DIR__ . '/Config/' . strtolower('Promotion') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Promotion'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Promotion')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Promotion'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Promotion')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Promotion'), __DIR__ . '/views');
