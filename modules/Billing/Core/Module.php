...
return new class implements ModuleContract {
    ...
    {
        $path = __DIR__ . '/Config/' . strtolower('Billing') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Billing'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Billing')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Billing'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Billing')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Billing'), __DIR__ . '/views');
