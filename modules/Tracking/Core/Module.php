...
return new class implements ModuleContract {
    ...
    {
        return 'Tracking/Core';
    }

    public function register(): void
    {
        $path = __DIR__ . '/Config/' . strtolower('Tracking') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Tracking'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Tracking')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Tracking'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Tracking')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Tracking'), __DIR__ . '/views');
